---
title: "Group Project"
author: "Group 5: Nithin Varma, Rajeev Varma and Nikitha Chigurupati"
date: "2023-05-01"
...
```{r}
train_v3 <- read.csv("~/Downloads/train_v3.csv")
```
...
#Normalize the "loss" column by dividing the percentages by 100
...
```{r}
# Create a visual plot of the AUC values versus the log of the lambda values.
plot(lasso_model_pd)
...
train_v3_complete <- cbind.data.frame(train_v3_preProcess, train_v3_preProcess_PCA)
```
...
myControl <- trainControl(method = "boot632", number = 5, repeats = 1, summaryFunction = twoClassSummary, classProbs = TRUE, verboseIter = TRUE, allowParallel = TRUE)
```
